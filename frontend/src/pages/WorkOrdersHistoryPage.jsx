...
const WorkOrdersHistoryPage = () => {
    ...
    const fetchData = async (silent = false) => {
        try {
            ...
            const getAxiosData = (index, defaultValue = []) => 
                results[index].status === 'fulfilled' ? results[index].value.data : defaultValue;

            const ordersData = getData(0);
            const assetsData = getData(1);
            const techsData = getData(2);
            const provsData = getData(3);
            const schedulesResData = getAxiosData(4);
            const mTypesResData = getAxiosData(5);
            const paymentTermsResData = getAxiosData(6);
            const locationsData = getData(7);

            if (Array.isArray(ordersData)) {
                setOrders(ordersData.sort((a, b) => b.id_ot - a.id_ot)); // Descending order usually better for history
            }
            
            setAssets(assetsData);
            setTechnicians(techsData);
            setProviders(provsData);
            setMaintenanceTypes(mTypesResData);
            setPendingSchedules(schedulesResData.filter(s => s.estado === 'programado' && !s.tiene_ot));
            setLocations(locationsData);
            setPaymentTerms(paymentTermsResData);
    ...
    const StatusSelector = ({ order }) => {
        ...
        const isAdmin = user?.role === 'ADMIN' || user?.role === 'SUPER-ADMIN' || user?.role === 'ADMINISTRADOR';
    ...
    const filteredOrders = orders.filter(order => {
        ...
        // ONLY show closed/cancelled orders in the history view
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-[#1e293b] p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-black text-slate-800 dark:text-white tracking-tight">HISTORIAL DE ÓRDENES DE TRABAJO</h2>
                        <p className="text-slate-500 text-sm font-medium">Archivo de mantenimientos completados</p>
                    </div>
                </div>
            </div>
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            {(user?.role === 'ADMIN' || user?.role === 'SUPER-ADMIN' || user?.role === 'ADMINISTRADOR') && (
                                                <>
                                                    <button
            ...
            >
                <div>
                    <div className="flex justify-between items-center mb-6">
                        {(user?.permisos?.includes('work_orders_link_maintenance') && !formData.id_calendario && (!formData.id_calendarios || formData.id_calendarios.length === 0)) && (
                            <button
                                type="button"
...
export default WorkOrdersHistoryPage;
