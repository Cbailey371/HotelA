        ...
        <div className="min-h-screen bg-slate-50 dark:bg-[#020617] text-slate-900 dark:text-slate-100 p-4 lg:p-8 font-sans transition-colors duration-500">
                ...
                <div className="flex flex-wrap items-center justify-center gap-3">
                    ...
                    <div className="bg-white dark:bg-slate-900 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-800 flex gap-1 shadow-sm">
                        <button
                            ...
                            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${view === 'month' ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/30' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}`}
                        >
                            Mes
                        </button>
                        <button
                            onClick={() => setView('week')}
                            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${view === 'week' ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/30' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}`}
                        >
                            Semana
                        </button>
                        <button
                            onClick={() => setView('day')}
                            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${view === 'day' ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/30' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}`}
                        >
                            Día
                        </button>
                    </div>
                    ...
                    <div className="flex items-center gap-2 bg-white dark:bg-slate-900 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                        <button onClick={() => navigate(-1)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors text-slate-500">
                            <ChevronLeft className="w-5 h-5" />
                        </button>
                        <span className="text-sm font-black text-slate-900 dark:text-white min-w-[140px] text-center uppercase tracking-tighter">
                    ...
                    <div className="relative group">
                        <div className="bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 px-4 py-3 rounded-2xl flex items-center gap-3 transition-transform active:scale-95 cursor-default">
                            <RefreshCcw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
                            <span className="text-[10px] font-black uppercase tracking-widest">
                                Refresco en: {refreshTime}s
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div className="space-y-6">
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden relative group">
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-tight">Tareas Hoy</div>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <div className="text-2xl font-black text-slate-900 dark:text-white">
                                        {events.filter(e => e.estado.toLowerCase() === 'completado').length}
                                    </div>
                                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-tight">Completados</div>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <div className="text-2xl font-black text-slate-900 dark:text-white">
                                        {events.filter(e => e.prioridad.toLowerCase() === 'alta' && e.estado.toLowerCase() !== 'completado').length}
                                    </div>
                                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-tight">Críticos Pendientes</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl">
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-[10px] font-black uppercase text-slate-600 dark:text-slate-400 italic">Programado</span>
                            </div>
                            <div className="flex items-center gap-3">
                                <span className="w-3 h-3 rounded-full bg-amber-500 shadow-lg shadow-amber-500/20"></span>
                                <span className="text-[10px] font-black uppercase text-slate-600 dark:text-slate-400 italic">En Proceso</span>
                            </div>
                            <div className="flex items-center gap-3">
                                <span className="w-3 h-3 rounded-full bg-emerald-500 shadow-lg shadow-emerald-500/20"></span>
                                <span className="text-[10px] font-black uppercase text-slate-600 dark:text-slate-400 italic">Completado</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-3">
                    {loading && events.length === 0 ? (
                        <div className="h-[600px] flex flex-col items-center justify-center bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl animate-pulse">
                            <div className="w-12 h-12 bg-blue-500/20 rounded-2xl mb-4 border-b-2 border-blue-500"></div>
                            <span className="text-sm font-black text-slate-400 uppercase tracking-widest">Sincronizando Calendario...</span>
                        </div>
                    ) : (
                        <div className="animate-in fade-in zoom-in duration-500">
                            {view === 'month' && renderMonthView()}
                            {view !== 'month' && (
                                <div className="h-[600px] flex flex-col items-center justify-center bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl italic text-slate-400">
                                    <CalendarRange className="w-12 h-12 mb-4 opacity-20" />
                                    <span className="text-sm font-black uppercase tracking-widest">Vista {view} en desarrollo</span>
                                </div>
                            )}
                        </div>
                    )}
            ...
            {/* Footer */}
            <div className="max-w-7xl mx-auto mt-8 flex justify-between items-center text-[10px] font-black text-slate-400 uppercase tracking-widest px-4 italic">
