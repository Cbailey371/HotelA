    ...
    let app = Router::new()
        ...
        .nest("/api", 
            Router::new()
                ...
                .route("/assets/import/create", post(controllers::assets::import_assets_create)
                    .layer(axum::middleware::from_fn_with_state(db.clone(), |state, req, next| middleware::auth::require_permission(state, req, next, "assets_edit")))
                )
                .route("/assets/import/update", post(controllers::assets::import_assets_update)
                    .layer(axum::middleware::from_fn_with_state(db.clone(), |state, req, next| middleware::auth::require_permission(state, req, next, "assets_edit")))
                )
                ...
                .route("/holidays", get(controllers::holidays::get_holidays).post(controllers::holidays::create_holiday))
