...
pub fn get_panama_variable_holidays(year: i32) -> Vec<(NaiveDate, String)> {
    let easter = calculate_easter(year);
    
    // Carnival is 47 days before Easter (Tuesday) and 48 days (Monday)
    let carnival_tuesday = easter - Duration::days(47);
    let carnival_monday = easter - Duration::days(48);
    
    // Good Friday is 2 days before Easter
    let good_friday = easter - Duration::days(2);
    
    vec![
        (carnival_monday, "Lunes de Carnaval".to_string()),
        (carnival_tuesday, "Martes de Carnaval".to_string()),
        (good_friday, "Viernes Santo".to_string()),
    ]
}
